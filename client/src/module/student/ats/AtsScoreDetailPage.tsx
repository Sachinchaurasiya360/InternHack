...
function ScoreCircle({ score }: { score: number }) {
  ...
  const bgColor = score >= 70 ? "bg-green-50 dark:bg-green-900/30" : score >= 40 ? "bg-yellow-50 dark:bg-yellow-900/30" : "bg-red-50 dark:bg-red-900/30";
  const ringColor = score >= 70 ? "ring-green-200 dark:ring-green-800" : score >= 40 ? "ring-yellow-200 dark:ring-yellow-800" : "ring-red-200 dark:ring-red-800";
  ...
  return (
    <div className={`w-36 h-36 rounded-full ${bgColor} ring-4 ${ringColor} flex flex-col items-center justify-center mx-auto`}>
      ...
      <span className="text-xs text-gray-500 dark:text-gray-500 mt-1">out of 100</span>
    </div>
  );
}
...
function CategoryBar({ label, score }: { label: string; score: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-10 text-sm font-semibold text-gray-700 dark:text-gray-300 text-right">{score}</span>
    </div>
  );
}
...
export default function AtsScoreDetailPage() {
  ...
  if (isLoading) return <div className="text-center py-16 text-gray-500 dark:text-gray-500">Loading...</div>;
  if (!score) return <div className="text-center py-16 text-gray-500 dark:text-gray-500">Score not found</div>;

  return (
    <div className="max-w-4xl mx-auto">
      <Link to="/student/ats/history" className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-500 hover:text-black dark:hover:text-white mb-6 no-underline">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">{score.jobTitle || "General ATS Score"}</h1>
        ...
        {/* Overall Score */}
        <div className="bg-white dark:bg-gray-900 p-8 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm text-center">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {score.keywordAnalysis.found.map((kw) => (
                  <span key={kw} className="px-3 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full text-xs font-medium">{kw}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {score.keywordAnalysis.missing.map((kw) => (
                  <span key={kw} className="px-3 py-1 bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded-full text-xs font-medium">{kw}</span>
                ))}
              </div>
            </div>
          )}
