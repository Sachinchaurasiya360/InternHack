...
/* ── Code Block (read-only with copy) ── */
...
function ExerciseSection({
  ...
  return (
    ...
    >
      ...
      {/* Starter code (read-only editor) */}
      <NodeEditor value={showSolution ? exercise.solution : exercise.starterCode} height="180px" />
      ...
      {exercise.expectedOutput && (
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-2.5 bg-gray-900">
            ...
            <pre className="text-sm text-emerald-400 whitespace-pre-wrap font-mono">{exercise.expectedOutput}</pre>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium rounded-xl transition-colors ${
            solved[exercise.id]
              ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
        ...
        >
          ...
          {solved[exercise.id] ? "Solved" : "Mark as Solved"}
        </button>
  ...
  return (
    <div className="relative pb-12">
      ...
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
        ...
        <div className="absolute -bottom-32 -left-32 w-125 h-125 bg-emerald-100 dark:bg-emerald-900/20 rounded-full blur-3xl opacity-40" />
        <div
        ...
        <div className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 px-6 py-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-green-50 dark:bg-green-900/30 flex items-center justify-center shrink-0">
              <span className="text-sm font-bold text-green-600 dark:text-green-400">{currentIndex + 1}</span>
            </div>
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${DIFF_BADGE[lesson.difficulty]}`}>
                  {lesson.difficulty}
                </span>
                {completed && (
      ...
      <div className="space-y-5">
        ...
        >
          {lesson.concepts.map((c) => (
            <span key={c} className="text-[10px] px-2.5 py-1 rounded-lg bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 font-medium">
              {c}
            </span>
          ))}
        </motion.div>
