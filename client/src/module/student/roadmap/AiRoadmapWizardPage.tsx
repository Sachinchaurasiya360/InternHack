...
export default function AiRoadmapWizardPage() {
  ...
  useEffect(() => {
    let mounted = true;
    api
      .get<{ enrollments: RoadmapEnrollmentListItem[] }>(
    ...
    api
      .get<{ used: number; limit: number; isPro: boolean }>(
        "/roadmaps/ai/usage",
      )
      .then((res) => {
        if (!mounted) return;
      ...
      .catch(() => {
        /* ok — hide counter if endpoint missing */
      });
    return () => {
      mounted = false;
    };
    ...
    try {
      ...
      const msg =
        (err as { response?: { data?: { message?: string } } }).response?.data
          ?.message ?? "Could not generate roadmap. Please try again.";
      toast.error(msg);
      setSubmitting(false);
    }
  };
