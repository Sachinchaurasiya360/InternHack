...
import type { FlaskProgress, CodeExample, PracticeExercise } from "./data/types";
  ...
  if (!lesson || !section) {
    return (
      <div className="text-center py-12">
        ...
        <Link to={basePath} className="text-blue-500 hover:underline text-sm mt-2 inline-block">Back to Flask Lessons</Link>
      </div>
    );
  }
      ...
      <div className="space-y-5">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 }} className="flex items-center gap-2 flex-wrap">
          {lesson.concepts.map((c) => (
            <span key={c} className="text-[10px] px-2.5 py-1 rounded-lg bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 font-medium">{c}</span>
          ))}
        </motion.div>
