  ...
  return (
    <div className="relative pb-12">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="text-center mb-8">
        ...
        <p className="text-lg text-gray-500 dark:text-gray-500 max-w-xl mx-auto">{section.description}</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="grid grid-cols-3 gap-4 mb-8">
        {[
          { icon: BookOpen, value: sectionLessons.length, label: "Lessons", iconColor: "text-blue-500" },
      ...
      <div className="space-y-3">
        {sectionLessons.map((lesson, i) => {
          ...
          return (
            <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.04 }}>
              <Link
              ...
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0">
                  {isCompleted ? (
                    ...
                    <span className="w-8 h-8 rounded-full border-2 border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 text-sm font-semibold">{i + 1}</span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className={`text-sm font-semibold ${isCompleted ? "text-gray-400 dark:text-gray-500 line-through" : "text-gray-900 dark:text-white"}`}>{lesson.title}</p>
                    {lesson.isInterviewQuestion && <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400 shrink-0" />}
                  </div>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {lesson.concepts.slice(0, 4).map((c) => (
                      <span key={c} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 font-medium">{c}</span>
                    ))}
                  </div>
                </div>
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full shrink-0 ${DIFF_BADGE[lesson.difficulty]}`}>{lesson.difficulty}</span>
