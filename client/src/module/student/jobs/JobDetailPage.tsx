...
export default function JobDetailPage() {
  ...
  if (isLoading) return <div className="min-h-screen bg-gray-50 dark:bg-gray-950"><Navbar /><div className="flex items-center justify-center h-64 pt-24 text-gray-500">Loading...</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-24 pb-12">
        <Link to="/jobs" className="flex items-center gap-2 text-sm text-gray-500 hover:text-black dark:hover:text-white mb-6 no-underline">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div>
                {isAuthenticated && user?.role === "STUDENT" ? (
                  <Link to={`/jobs/${job.id}/apply`}
                    className="px-6 py-3 bg-black text-white font-semibold rounded-xl hover:bg-gray-800 transition-colors no-underline text-sm dark:bg-white dark:text-gray-950 dark:hover:bg-gray-200">
                    Apply Now
                  </Link>
                ) : !isAuthenticated ? (
                  <Link to="/login"
          ...
          {job.rounds && job.rounds.length > 0 && (
            <div className="bg-white dark:bg-gray-900 p-8 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm mb-6">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Hiring Process</h2>
              <div className="space-y-4">
                {job.rounds.map((round, i) => (
                  <div key={round.id} className="flex items-start gap-4">
                    <div className="flex flex-col items-center">
                      <div className="w-8 h-8 bg-black text-white rounded-full flex items-center justify-center text-sm font-bold dark:bg-white dark:text-gray-950">{i + 1}</div>
                      {i < job.rounds!.length - 1 && <div className="w-px h-8 bg-gray-200 dark:bg-gray-700 mt-1" />}
                    </div>
                    <div className="flex-1 pb-2">
                      ...
                      {round.customFields.length > 0 && (
                        <span className="text-xs text-gray-400 dark:text-gray-500 mt-1 block">{round.customFields.length} field(s) to complete</span>
                      )}
                    </div>
                    <CheckCircle className="w-5 h-5 text-gray-300 dark:text-gray-600 mt-1" />
                  </div>
                ))}
              </div>
            </div>
          ...
          {job.recruiter && (
            <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Posted By</h2>
