  ...
  if (!lesson || !section) {
    return (
      <div className="text-center py-12">
        ...
        <Link to={basePath} className="text-blue-500 hover:underline text-sm mt-2 inline-block">Back to Django Lessons</Link>
      </div>
    );
  }
  ...
  return (
    <div className="relative pb-12">
      ...
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
        ...
        <div className="absolute -bottom-32 -left-32 w-125 h-125 bg-green-100 dark:bg-green-900/20 rounded-full blur-3xl opacity-40" />
        ...
        <div className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 px-6 py-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center shrink-0">
              <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">{currentIndex + 1}</span>
            </div>
      ...
      <div className="space-y-5">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.05 }} className="flex items-center gap-2 flex-wrap">
          {lesson.concepts.map((c) => (
            <span key={c} className="text-[10px] px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 font-medium">{c}</span>
          ))}
        </motion.div>
