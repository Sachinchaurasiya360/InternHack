...
import type { DataAnalyticsProgress, CodeExample, PracticeExercise } from "./data/types";
    ...
    return (
      <div className="text-center py-12">
        ...
        <Link to={basePath} className="text-blue-500 hover:underline text-sm mt-2 inline-block">
          Back to Data Analytics Lessons
        </Link>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 px-6 py-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-teal-50 dark:bg-teal-900/30 flex items-center justify-center shrink-0">
              <span className="text-sm font-bold text-teal-600 dark:text-teal-400">{currentIndex + 1}</span>
            </div>
      ...
      <div className="space-y-5">
        ...
        >
          {lesson.concepts.map((c) => (
            <span key={c} className="text-[10px] px-2.5 py-1 rounded-lg bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 font-medium">
              {c}
            </span>
          ))}
        </motion.div>
