...
export default function RecruiterDashboard() {
  ...
  const stats = [
    { label: "Total Jobs", value: data.totalJobs, icon: Briefcase, color: "bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" },
    { label: "Active Jobs", value: data.activeJobs, icon: TrendingUp, color: "bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400" },
    { label: "Total Applicants", value: data.totalApplications, icon: Users, color: "bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400" },
    { label: "Hired", value: data.hiredCount, icon: Award, color: "bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400" },
  ];
    ...
    <div>
      ...
      {Object.keys(data.statusBreakdown).length > 0 && (
        <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm mb-8">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Application Status Breakdown</h2>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
        ...
        {data.recentApplications.length === 0 ? (
          <div className="p-8 text-center text-gray-500 dark:text-gray-500">
            No applications yet. <Link to="/recruiters/jobs/create" className="text-black dark:text-white font-medium hover:underline">Create a job</Link> to get started.
          </div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {data.recentApplications.map((app) => (
              <Link
                ...
                className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors no-underline"
              >
                <div>
                  <p className="font-medium text-gray-900 dark:text-white">{app.student.name}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-500">{app.job.title}</p>
                </div>
                <div className="text-right">
                  <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(app.status)}`}>
                    {app.status}
                  </span>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{new Date(app.createdAt).toLocaleDateString()}</p>
                </div>
              </Link>
            ))}
          </div>
