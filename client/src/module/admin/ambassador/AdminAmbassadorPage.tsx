...
export default function AdminAmbassadorPage() {
  ...
  const [ambassadorDetail, setAmbassadorDetail] = useState<any>(null);
  ...
  const reviewShareMutation = useMutation({
    ...
    onError: () => toast.error("Failed to review share"),
  });
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-white mb-6">OSS Ambassador Program</h1>
                ...
                <div className="flex gap-4 mb-6">
                  <div className="relative flex-1 max-w-md">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                    />
                  </div>
                  <select
                    value={statusFilter}
                    onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
                    className="bg-gray-900 border border-gray-700 rounded-lg text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                      ...
                      <tbody>
                        {ambassadorsQuery.isLoading ? (
                          ...
                          ambassadorsQuery.data?.ambassadors?.map((a: Ambassador) => (
                            <tr key={a.id} className="border-b border-gray-800 hover:bg-gray-800/50 cursor-pointer"
...
function AmbassadorDetailView({
  ...
  return (
    <div>
      <button onClick={onBack} className="text-gray-400 hover:text-white text-sm mb-4">&larr; Back to list</button>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className={`font-medium ${ambassador.status === "APPROVED" ? "text-emerald-400" : ambassador.status === "REJECTED" ? "text-red-400" : "text-amber-400"}`}>{ambassador.status}</p>
              </div>
              <div>
                <span className="text-gray-400">Guides Completed</span>
                <p className="text-white">{ambassador.guidesCompleted}/6</p>
              </div>
              <div>
                <span className="text-gray-400">Repos Contributed</span>
              ...
              <div>
                <span className="text-gray-400">Account Age (days)</span>
                <p className="text-white">{ambassador.accountAgeDays}</p>
              </div>
              <div>
                <span className="text-gray-400">Premium Granted</span>
          ...
          {ambassador.referralLinks?.length > 0 && (
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-6">
              ...
              <div className="space-y-3">
                {ambassador.referralLinks.map((link: any) => (
                  <div key={link.id} className="flex items-center justify-between bg-gray-800 rounded-lg px-4 py-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <Link2 className="w-4 h-4 text-gray-400 shrink-0" />
                        <code className="text-sm text-indigo-400 truncate">{link.url}</code>
                      </div>
                      {link.label && <p className="text-xs text-gray-400 mt-1">{link.label}</p>}
                    </div>
                    <div className="flex items-center gap-4 shrink-0 ml-4">
                      <span className="text-xs text-gray-400">{link.clicks} clicks</span>
                      <span className="text-xs text-gray-400">{link._count?.conversions ?? 0} conversions</span>
                      <button onClick={() => onCopy(link.url)} className="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white">
                        <Copy className="w-3.5 h-3.5" />
                      </button>
                      <a href={link.url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded hover:bg-gray-700 text-gray-400 hover:text-white">
          ...
          {ambassador.shares?.length > 0 && (
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-6">
              ...
              <div className="space-y-3">
                {ambassador.shares.map((share: Share) => (
                  <div key={share.id} className="flex items-center justify-between bg-gray-800 rounded-lg px-4 py-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <Share2 className="w-4 h-4 text-gray-400" />
                        <span className="text-white text-sm font-medium">{share.platform}</span>
                        <span className={`text-xs px-1.5 py-0.5 rounded ${
                          share.status === "APPROVED" ? "bg-emerald-500/10 text-emerald-400" :
                          share.status === "REJECTED" ? "bg-red-500/10 text-red-400" :
                          "bg-amber-500/10 text-amber-400"
                        }`}>{share.status}</span>
                      </div>
                      <a href={share.url} target="_blank" rel="noopener noreferrer" className="text-indigo-400 text-xs hover:underline mt-1 inline-block">{share.url}</a>
                      {share.description && <p className="text-gray-400 text-xs mt-1">{share.description}</p>}
                    </div>
...
function SharesTab({ ambassadors, onReview, onCopy }: {
  ...
  return (
    <div>
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-400 mb-2">Select Ambassador</label>
        <select
          value={selectedAmbId ?? ""}
          onChange={(e) => setSelectedAmbId(e.target.value ? parseInt(e.target.value) : null)}
          className="bg-gray-900 border border-gray-700 rounded-lg text-white px-3 py-2 text-sm w-full max-w-md"
        >
      ...
      {selectedAmbId && (
        <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
          {sharesQuery.isLoading ? (
            ...
            <div className="py-12 text-center text-gray-500">No shares found</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800 text-gray-400">
              ...
              <tbody>
                {sharesQuery.data?.shares?.map((share: Share) => (
                  <tr key={share.id} className="border-b border-gray-800">
                    <td className="px-4 py-3 text-white">{share.platform}</td>
                    <td className="px-4 py-3">
                      <a href={share.url} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline text-xs break-all">{share.url}</a>
                      <button onClick={() => onCopy(share.url)} className="ml-2 text-gray-400 hover:text-white"><Copy className="w-3 h-3 inline" /></button>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                        share.status === "APPROVED" ? "bg-emerald-500/10 text-emerald-400" :
...
function SpotlightsTab({ spotlights, ambassadors, isLoading }: {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => toast.error("Failed to delete spotlight"),
  });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <p className="text-gray-400 text-sm">{spotlights.length} spotlight(s)</p>
      ...
      {showForm && (
        <div className="bg-gray-900 rounded-lg border border-gray-800 p-6 mb-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Ambassador *</label>
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center text-gray-500">No spotlights yet</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {spotlights.map((s) => (
            <div key={s.id} className="bg-gray-900 rounded-lg border border-gray-800 p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-gray-400 text-xs">{s.month}/{s.year}</div>
                  </div>
                </div>
                <button onClick={() => deleteMutation.mutate(s.id)} className="text-gray-500 hover:text-red-400">
              ...
              <div className="mt-2">
                <span className={`text-xs px-1.5 py-0.5 rounded ${s.isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"}`}>
  ...
  return (
    <div>
      {isLoading ? (
        ...
        <div className="py-12 text-center text-gray-500">No referrals this month yet</div>
      ) : (
        <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              ...
              {referrers.map((r, i) => (
                <tr key={r.userId} className="border-b border-gray-800">
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <span className="text-gray-400 text-xs ml-1">signups</span>
                  </td>
                </tr>
              ))}
            </tbody>
