        ...
        {Array.from({ length: rows }).map((_, r) => (
          <RowCells key={`r-${r}`} r={r} rowLabels={rowLabels}>
            {frame.dp[r].map((v, c) => {
              ...
              let color = C.text;
              if (isPath) { bg = `${C.danger}24`; border = `2px solid ${C.danger}`; color = C.dangerDark; }
              else if (isJust) { bg = `${C.success}24`; border = `2px solid ${C.success}`; color = C.successDark; }
              else if (isDep) { bg = "#FEF3C7"; border = "2px solid #F59E0B"; color = "#B45309"; }
              return (
                <div key={key} style={{
                  display: "flex", alignItems: "center", justifyContent: "center",
...
function VisualizeTab() {
      ...
      <AlgoCanvas
        ...
        legend={
          <div style={{ display: "flex", gap: 14, fontSize: "0.72rem" }}>
            <Legend color={C.success} label="just filled" />
            <Legend color="#F59E0B" label="dependency" />
            <Legend color={C.danger} label="LCS path (backtrack)" />
          </div>
        }
      >
  ...
  return (
    <AlgoCanvas
      ...
      legend={<div style={{ fontSize: "0.72rem", color: C.textMuted }}>dp[i][j] = min scalar mults to multiply M[i..j].</div>}
    >
      {frame ? (
        <DPGrid frame={frame} cellSize={42} rowLabels={Array.from({ length: n }, (_, i) => `${i}`)} colLabels={Array.from({ length: n }, (_, i) => `${i}`)} />
      ) : <div style={{ color: C.textMuted }}>Need at least 2 dimensions.</div>}
    </AlgoCanvas>
  );
}
...
function TryTab() {
  ...
  return (
    <div style={{ fontFamily: C.heading, display: "flex", flexDirection: "column", gap: 12 }}>
      <Callout accent={A}>Draw the DP table on paper first — then verify here.</Callout>
      {problems.map((p, i) => {
        const g = guesses[i];
        ...
        return (
          <Card key={i}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              ...
              <span style={{ flex: 1, fontSize: "0.88rem", color: C.text }}>{p.q}</span>
              <input
                type="text"
                placeholder="answer"
                value={g ?? ""}
            ...
            {revealed && <div style={{ fontSize: "0.78rem", color: C.textMuted, marginTop: 8 }}>Hint: {p.hint}</div>}
          </Card>
        );
      })}
    </div>
    ...
    <div style={{ fontFamily: C.heading, display: "flex", flexDirection: "column", gap: 12 }}>
      <Card>
        ...
        <p style={{ fontSize: "0.9rem", color: C.textSecondary, lineHeight: 1.6, margin: 0 }}>
          ...
          In MCM you fill by increasing chain length, because dp[i][j] depends on strictly
          shorter intervals.
        </p>
      </Card>
      <Card>
        ...
        <p style={{ fontSize: "0.9rem", color: C.textSecondary, lineHeight: 1.6, margin: 0 }}>
          O(nW) looks polynomial, but W is a <em>numeric value</em> — its encoding is log(W)
          bits. So the real complexity is exponential in the input size. 0/1 knapsack is NP-hard;
