...
export class StudyBuddyService {
  ...
  async optOut(userId: number, roadmapId: number) {
    ...
    await this.deactivatePairAndRematchBuddy(userId, roadmapId);
  }
  ...
  async findAndCreateMatch(userId: number, roadmapId: number) {
    ...
    const preference = await this.getPreference(userId, roadmapId);
    ...
    return prisma.roadmapStudyBuddyPair.create({
      data: {
        roadmapId,
        studentAId: userId,
